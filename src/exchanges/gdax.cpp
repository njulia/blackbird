...
bool arbitrage(Parameters& params)
{
  ...
  if (!get_orderbook(params, ccy_pair, btcusd_bid_price, btcusd_bid_size, btcusd_ask_price, btcusd_ask_size))
      return false; 
  // ETH-BTC
  ccy_pair = "ETH-BTC";
  double ethbtc_bid_price, ethbtc_bid_size, ethbtc_ask_price, ethbtc_ask_size;
  if (!get_orderbook(params, ccy_pair, ethbtc_bid_price, ethbtc_bid_size, ethbtc_ask_price, ethbtc_ask_size))
      return false; 
  // ETH/USD
  ccy_pair = "ETH-EUR";
  double ethusd_bid_price, ethusd_bid_size, ethusd_ask_price, ethusd_ask_size;
  if (!get_orderbook(params, ccy_pair, ethusd_bid_price, ethusd_bid_size, ethusd_ask_price, ethusd_ask_size))
      return false;; 

  float fees = params.gdaxFees;
  static float account = 1000.0;
  float min_balance = 50.0;
  float potential_pnl = account / btcusd_ask_price / ethbtc_ask_price * ethusd_bid_price - 3*fees*account - account;
  cout << "Potential buy BTC, sell ETH PNL=" << potential_pnl << endl;
  *params.logFile << "Potential buy BTC, sell ETH PNL=" << potential_pnl << endl;
  // Arbitrage when there is profilt
  if (potential_pnl > account * 0.001)
  {
    account += potential_pnl;
    cout << "Opportunity: buy BTC / sell ETH PNL=" << potential_pnl << " Account=" << account << endl;
    *params.logFile << "Opportunity: buy BTC / sell ETH PNL=" << potential_pnl << " Account=" << account << endl;
    double usd_balance = 0.0; //getAvail(params, "EUR");
    ...
    double buy_power  = min(min(min(btcusd_ask_size*btcusd_ask_price, ethbtc_ask_size*ethbtc_ask_price), ethusd_bid_size*ethusd_bid_price), usd_balance); //Only use 90% fund 
    ...
    *params.logFile << "Buy BTC-EUR Done" << endl;
    ...
    auto ethbtc_id = sendLimitOrder(params, "buy", ethbtc_size, ethbtc_price, "ETH-BTC"); 
    sleep_for(millisecs(5000));
    is_order_complete = isOrderComplete(params, ethbtc_id);
    while (!is_order_complete)
    {
      sleep_for(millisecs(5000));
      cout << "Buy ETH-BTC " << ethbtc_price << " @ " << ethbtc_size << " still open..." << endl;
      is_order_complete = isOrderComplete(params, ethbtc_id);
    }
    // Reset the order id
    ethbtc_id ="0";
    cout << "Buy ETH-BTC Done" << endl;
    *params.logFile << "Buy ETH-BTC Done" << endl;
    ...
    auto ethusd_id = sendLimitOrder(params, "sell", ethusd_size, ethusd_price, "ETH-EUR"); 
    sleep_for(millisecs(5000));
    is_order_complete = isOrderComplete(params, ethusd_id);
    while (!is_order_complete)
    {
      sleep_for(millisecs(5000));
      cout << "Sell ETH-EUR " << ethusd_price << " @ " << ethusd_size << " still open..." << endl;
      is_order_complete = isOrderComplete(params, ethusd_id);
    }
    // Reset the order id
    ethusd_id ="0";
    cout << "Sell ETH-EUR Done" << endl;
    *params.logFile << "Sell ETH-EUR Done" << endl;

    // Check the pnl of this arbitrage
    double new_balance = getAvail(params, "EUR");
    double pnl = new_balance - usd_balance;;
    account += pnl;
    std::cout << "Buy BTC -> buy ETH/BTC -> sell ETH, PNL=" << pnl << " New balance=$" << new_balance << std::endl;
    *params.logFile << "Buy BTC -> buy ETH -> sell ETH, PNL=" << pnl << " New balance=$" << new_balance << std::endl;
    return true;
  }
  else
  {
    potential_pnl = account / ethusd_ask_price * ethbtc_bid_price * btcusd_bid_price - 3*fees*account - account;
    cout << "Potential buy ETH, sell BTC PNL=" << potential_pnl << endl;
    *params.logFile << "Potential buy ETH, sell BTC PNL=" << potential_pnl << endl;
    if (potential_pnl < account * 0.001)
    {
       return false;
    }
    account += potential_pnl;
    cout << "Opportunity: buy ETH / sell BTC PNL=" << potential_pnl << " Account=" << account << endl;
    *params.logFile << "Opportunity: buy ETH / sell BTC PNL=" << potential_pnl << " Account=" << account << endl;
    double usd_balance = getAvail(params, "EUR");
    if (usd_balance < min_balance)
    {
      cout << "Balance " << usd_balance << " < " << account << endl;
      return false;
    }
    // Check the maximum fund can be used for arbitrage. Should be less than the top order book, and less than the account balance
    double buy_power  = min(min(min(ethusd_ask_size*ethusd_ask_price, ethbtc_bid_size*ethbtc_bid_price), btcusd_bid_size*btcusd_bid_price), usd_balance); //Only use 90% fund 
    ...
    auto ethusd_id = sendLimitOrder(params, "buy", ethusd_size, ethusd_price, "ETH-EUR"); 
    sleep_for(millisecs(5000));
    bool is_order_complete = isOrderComplete(params, ethusd_id);
    while (!is_order_complete)
    {
      sleep_for(millisecs(5000));
      cout << "Buy ETH-EUR " << ethusd_price << " @ " << ethusd_size << " still open..." << endl;
      is_order_complete = isOrderComplete(params, ethusd_id);
    }
    // Reset the order id
    ethusd_id ="0";
    cout << "Buy ETH-EUR Done" << endl;
    *params.logFile << "Buy ETH-EUR Done" << endl;
    ...
    auto ethbtc_id = sendLimitOrder(params, "sell", ethbtc_size, ethbtc_price, "ETH-BTC"); 
    sleep_for(millisecs(5000));
    is_order_complete = isOrderComplete(params, ethbtc_id);
    while (!is_order_complete)
    {
      sleep_for(millisecs(5000));
      cout << "Sell ETH-BTC " << ethbtc_price << " @ " << ethbtc_size << " still open..." << endl;
      is_order_complete = isOrderComplete(params, ethbtc_id);
    }
    // Reset the order id
    ethbtc_id ="0";
    cout << "Sell ETH-BTC Done" << endl;
    *params.logFile << "Sell ETH-BTC Done" << endl;
    ...
    auto btcusd_id = sendLimitOrder(params, "sell", btcusd_size, btcusd_price, "BTC-EUR"); 
    sleep_for(millisecs(5000));
    is_order_complete = isOrderComplete(params, btcusd_id);
    while (!is_order_complete)
    {
    ...
    // Reset the order id
    btcusd_id ="0";
    cout << "Sell BTC-EUR Done" << endl;
    *params.logFile << "Sell BTC-EUR Done" << endl;
    ...
    double pnl = new_balance - usd_balance;;
    account += pnl;
    std::cout << "Buy ETH -> sell ETH/BTC -> sell BTC, PNL=" << pnl << " New balance=$" << new_balance << std::endl;
    *params.logFile << "Buy ETH -> sell ETH/BTC -> sell BTC, PNL=" << pnl << " New balance=$" << new_balance << std::endl;
    return true;
  }
  return false;
}
...
bool macd(Parameters& params)
{
  ...
  if (!get_orderbook(params, ccy_pair, btceur_bid_price, btceur_bid_size, btceur_ask_price, btceur_ask_size))
      return false;
  ...
  if (btc_signal > 0)
  {
    ...
    *params.logFile << "Buy signal: BTC-EUR EUR=" << account_btc << " BTC=" << btc_balance << endl;
    //double eur_balance = getAvail(params, "EUR");
    // Buy BTC-EUR order
    double ticksize = 0.01;
  ...
  if (!get_orderbook(params, ccy_pair, etheur_bid_price, etheur_bid_size, etheur_ask_price, etheur_ask_size))
      return false; 
